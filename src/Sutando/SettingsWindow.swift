...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    // Wave 4.10 — Skills section
    private var skillsSectionContainer: NSStackView?
    private var skillsSummaryLabel: NSTextField?
    private var skillsInstalledStack: NSStackView?
    private var skillsCloudToolsStack: NSStackView?
    private var skillsLocalStack: NSStackView?
    private var skillsBuiltinStack: NSStackView?
    private var skillsBuiltinDisclosure: NSButton?
    private var skillsInstalledHeader: NSTextField?
    private var skillsCloudToolsHeader: NSTextField?
    private var skillsLocalHeader: NSTextField?
    private var lastInventoryFetchTs: TimeInterval = 0
    private var lastInventory: CloudClient.CloudInventory?
    private var pendingBuiltinRebuild = true
    ...
    private func buildUI() {
        ...
        // Skills — what Sutando can do (Wave 4.10). Combines local file
        // walks (built-in, private) with /api/me/inventory (installed
        // from Station + activated cloud tools).
        stack.addArrangedSubview(sectionHeader("Skills"))
        stack.addArrangedSubview(skillsSection())
    ...
    /// One row in the Settings → Skills section. Comes from either a
    /// local manifest.json walk (built-in / private / learned) or from
    /// /api/me/inventory (installed-from-Station / cloud-tools).
    private struct LocalSkillInfo {
        let slug: String          // directory name
        let name: String          // manifest.name, falls back to slug
        let description: String   // manifest.description, "" if absent
        let path: String          // absolute path to the skill dir
    }

    private func skillsSection() -> NSView {
        let outer = NSStackView()
        outer.orientation = .vertical
        outer.alignment = .leading
        outer.spacing = 10
        ...
        let summary = NSTextField(labelWithString: "Counting skills…")
        summary.font = .systemFont(ofSize: 12)
        summary.textColor = .secondaryLabelColor
        skillsSummaryLabel = summary
        topRow.addArrangedSubview(summary)
        ...
        let browse = NSButton(
            ...
            target: self,
            action: #selector(openStationCatalog)
        )
        browse.bezelStyle = .rounded
        topRow.addArrangedSubview(browse)

        let publish = NSButton(
            title: "Publish a skill →",
            target: self,
            action: #selector(openStationPublish)
        )
        publish.bezelStyle = .rounded
        topRow.addArrangedSubview(publish)
        outer.addArrangedSubview(topRow)

        // Group 1: Installed from Station.
        let installedHeader = skillsGroupHeader("Installed from Station")
        skillsInstalledHeader = installedHeader
        outer.addArrangedSubview(installedHeader)
        let installedStack = NSStackView()
        installedStack.orientation = .vertical
        installedStack.alignment = .leading
        installedStack.spacing = 6
        installedStack.translatesAutoresizingMaskIntoConstraints = false
        installedStack.widthAnchor.constraint(equalToConstant: 504).isActive = true
        skillsInstalledStack = installedStack
        outer.addArrangedSubview(installedStack)

        // Group 2: Cloud tools activated.
        let cloudHeader = skillsGroupHeader("Cloud tools activated")
        skillsCloudToolsHeader = cloudHeader
        outer.addArrangedSubview(cloudHeader)
        let cloudStack = NSStackView()
        cloudStack.orientation = .vertical
        cloudStack.alignment = .leading
        cloudStack.spacing = 6
        cloudStack.translatesAutoresizingMaskIntoConstraints = false
        cloudStack.widthAnchor.constraint(equalToConstant: 504).isActive = true
        skillsCloudToolsStack = cloudStack
        outer.addArrangedSubview(cloudStack)

        // Group 3: Local (private to this Mac).
        let localHeader = skillsGroupHeader("Local (private to this Mac)")
        skillsLocalHeader = localHeader
        outer.addArrangedSubview(localHeader)
        let localStack = NSStackView()
        localStack.orientation = .vertical
        localStack.alignment = .leading
        localStack.spacing = 6
        localStack.translatesAutoresizingMaskIntoConstraints = false
        localStack.widthAnchor.constraint(equalToConstant: 504).isActive = true
        skillsLocalStack = localStack
        outer.addArrangedSubview(localStack)

        // Group 4: Built-in (always available, collapsed).
        let disclosure = NSButton()
        disclosure.bezelStyle = .recessed
        ...
        disclosure.font = .systemFont(ofSize: 12, weight: .medium)
        disclosure.target = self
        disclosure.action = #selector(toggleBuiltinSkills)
        skillsBuiltinDisclosure = disclosure
        outer.addArrangedSubview(disclosure)

        let builtinStack = NSStackView()
        builtinStack.orientation = .vertical
        builtinStack.alignment = .leading
        builtinStack.spacing = 4
        builtinStack.translatesAutoresizingMaskIntoConstraints = false
        builtinStack.widthAnchor.constraint(equalToConstant: 504).isActive = true
        builtinStack.isHidden = true
        skillsBuiltinStack = builtinStack
        outer.addArrangedSubview(builtinStack)

        // Footer note about propagation timing — sets user expectation.
        let footer = NSTextField(labelWithString:
            "Dashboard installs sync within ~1 min via background loop; new skills appear here after the next refresh. Voice-agent reload picks up new tools on its next session.")
        footer.font = .systemFont(ofSize: 10)
        footer.textColor = .tertiaryLabelColor
        footer.maximumNumberOfLines = 2
        footer.preferredMaxLayoutWidth = 504
        outer.addArrangedSubview(footer)

        skillsSectionContainer = outer
        return outer
    }

    private func skillsGroupHeader(_ title: String) -> NSTextField {
    ...
    /// Refresh the Skills section data sources. Local file walks are
    /// cheap (one stat + small JSON parse per skill dir); the cloud
    /// inventory fetch is throttled to 30s while the window is open.
    private func refreshSkillsSection() {
        let now = Date().timeIntervalSince1970
        let cloudThrottleMet = now - lastInventoryFetchTs >= 30.0

        // Always re-walk local + built-in (sub-100ms typical) so the
        // section reflects file-system changes (e.g., new local skill
        // dropped in $SUTANDO_PRIVATE_DIR/skills/).
        applyLocalSkills()
        applyBuiltinSkills()
        ...
        if cloudThrottleMet || lastInventory == nil {
    ...
    private func applyCloudInventory(inventory: CloudClient.CloudInventory?) {
        guard let installedStack = skillsInstalledStack,
              let cloudStack = skillsCloudToolsStack,
              let installedHeader = skillsInstalledHeader,
              let cloudHeader = skillsCloudToolsHeader else { return }

        installedStack.arrangedSubviews.forEach { $0.removeFromSuperview() }
        cloudStack.arrangedSubviews.forEach { $0.removeFromSuperview() }

        let installed = inventory?.installed ?? []
        let tools = inventory?.cloudTools ?? []

        installedHeader.stringValue = "Installed from Station  (\(installed.count))"
        cloudHeader.stringValue = "Cloud tools activated  (\(tools.count))"
        ...
        if installed.isEmpty {
            ...
            installedStack.addArrangedSubview(empty)
        } else {
            for skill in installed {
                installedStack.addArrangedSubview(makeInstalledRow(skill))
            }
        }
        ...
        if tools.isEmpty {
            ...
            empty.font = .systemFont(ofSize: 11)
            empty.textColor = .tertiaryLabelColor
            cloudStack.addArrangedSubview(empty)
        } else {
            for tool in tools {
                cloudStack.addArrangedSubview(makeCloudToolRow(tool))
            }
        }
    }

    private func applyLocalSkills() {
        guard let localStack = skillsLocalStack,
              let localHeader = skillsLocalHeader else { return }
        localStack.arrangedSubviews.forEach { $0.removeFromSuperview() }
        let entries = walkLocalSkills()
        localHeader.stringValue = "Local (private to this Mac)  (\(entries.count))"
        if entries.isEmpty {
            let hint = NSTextField(labelWithString:
                "None yet. Drop a skill dir at $SUTANDO_PRIVATE_DIR/skills/<slug>/ — Sutando picks it up on next voice-agent restart.")
            hint.font = .systemFont(ofSize: 11)
            hint.textColor = .tertiaryLabelColor
            hint.maximumNumberOfLines = 2
            hint.preferredMaxLayoutWidth = 504
            localStack.addArrangedSubview(hint)
        } else {
            for entry in entries {
                localStack.addArrangedSubview(makeLocalRow(entry))
            }
        }
    }

    private func applyBuiltinSkills() {
        guard let builtinStack = skillsBuiltinStack,
              let disclosure = skillsBuiltinDisclosure else { return }
        if !pendingBuiltinRebuild && builtinStack.arrangedSubviews.count > 0 {
            // Already populated; just refresh the disclosure title.
            return
        }
        pendingBuiltinRebuild = false
        builtinStack.arrangedSubviews.forEach { $0.removeFromSuperview() }
        let entries = walkBuiltinSkills()
        let prefix = builtinStack.isHidden ? "▶" : "▼"
        disclosure.title = "\(prefix) Built-in  (\(entries.count) — always available)"
        for entry in entries {
            let row = NSStackView()
            row.orientation = .horizontal
            ...
            builtinStack.addArrangedSubview(row)
        }
    }

    private func updateSkillsSummary() {
        guard let summary = skillsSummaryLabel else { return }
        ...
        let signed = CloudAuth.shared.isSignedIn
        if signed {
            summary.stringValue = String(
                format: "%d built-in · %d installed · %d cloud tools · %d local",
                builtin, installed, tools, local)
        } else {
            summary.stringValue = String(format: "%d built-in · %d local · sign in for Station inventory", builtin, local)
        }
    }

    private func makeInstalledRow(_ skill: CloudClient.CloudInstalledSkill) -> NSView {
        let row = NSStackView()
        row.orientation = .horizontal
        row.alignment = .centerY
        row.spacing = 8
        ...
        let label = NSTextField(labelWithString: "\(skill.name)  v\(skill.version)")
        label.font = .systemFont(ofSize: 12, weight: .medium)
        row.addArrangedSubview(label)
        ...
        let uninstall = NSButton(title: "Uninstall", target: self, action: #selector(uninstallSkillFromRow(_:)))
    ...
    private func makeCloudToolRow(_ tool: CloudClient.CloudActivatedTool) -> NSView {
        let outer = NSStackView()
        outer.orientation = .vertical
        outer.alignment = .leading
        outer.spacing = 2
        ...
        let label = NSTextField(labelWithString: tool.name)
        label.font = .systemFont(ofSize: 12, weight: .medium)
        head.addArrangedSubview(label)
        ...
        let deactivate = NSButton(title: "Deactivate", target: self, action: #selector(uninstallSkillFromRow(_:)))
        deactivate.bezelStyle = .rounded
        deactivate.controlSize = .small
        deactivate.font = .systemFont(ofSize: 11)
        ...
        meter.font = .systemFont(ofSize: 10)
    ...
    private func makeLocalRow(_ skill: LocalSkillInfo) -> NSView {
        let row = NSStackView()
        row.orientation = .horizontal
        row.alignment = .centerY
        row.spacing = 8
        ...
        let open = NSButton(title: "Open folder", target: self, action: #selector(openLocalSkillFromRow(_:)))
    ...
    // MARK: - Skills section actions

    @objc private func openStationCatalog() {
        let url = (CloudAuth.shared.record()?.apiBase ?? "https://sutando.ag2.ai") + "/superpower"
        if let u = URL(string: url) { NSWorkspace.shared.open(u) }
    }

    @objc private func openStationPublish() {
        let url = (CloudAuth.shared.record()?.apiBase ?? "https://sutando.ag2.ai") + "/superpower/publish"
        if let u = URL(string: url) { NSWorkspace.shared.open(u) }
    }

    @objc private func toggleBuiltinSkills() {
        guard let stack = skillsBuiltinStack, let disclosure = skillsBuiltinDisclosure else { return }
        stack.isHidden.toggle()
        let symbol = stack.isHidden ? "▶" : "▼"
        let count = walkBuiltinSkills().count
        disclosure.title = "\(symbol) Built-in  (\(count) — always available)"
    }

    @objc private func uninstallSkillFromRow(_ sender: NSButton) {
    ...
    @objc private func openLocalSkillFromRow(_ sender: NSButton) {
        guard let path = sender.identifier?.rawValue else { return }
        NSWorkspace.shared.selectFile(path, inFileViewerRootedAtPath: "")
    }

    // MARK: - Skill file walks

    private var builtinSkillsDir: String {
        // When bundled, skills live at Resources/repo/skills. Dev raw
        // build falls through to the repo's skills/ directory.
        ...
        // Dev: workspace root + /skills. Reuse the same walk logic as
        // main.swift's resolveWorkspace, but a simpler heuristic
        // (CWD ancestor with skills/) is enough for Settings display.
    ...
    /// Walk a skill directory: for each subdir, parse its manifest.json
    /// to extract name + description. Return entries sorted by name.
    /// Soft-fails on parse errors (skill is silently omitted).
        ...
        for entry in entries {
            ...
            if let data = try? Data(contentsOf: URL(fileURLWithPath: manifestPath)),
               let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                ...
                // Skip skills explicitly disabled in the manifest — they
                // aren't loaded by inline-tools.ts so showing them here
                // would mislead the user about Sutando's actual surface.
    ...
    private func startStatusPolling() {
        ...
        Timer.scheduledTimer(withTimeInterval: 1.5, repeats: true) { [weak self] timer in
            ...
            // refreshSkillsSection throttles cloud fetch to 30s; local
            // walks run on every tick (cheap; sub-100ms typical).
            self.refreshSkillsSection()
        }
    }

 ...
 * → discord_voice, otherwise voice) and recordSession's lifecycle-event
 * fan-out routes by `source`. Per-tool-call rows are written in real
 * time via `recordToolCall()` instead of being batched at session end.
 *
 ...
 *
 * Ported in Phase 5.20 of the OSS → private sync. Stando intentionally
 * preserves the lazy `require('node:sqlite')` path inside init() rather
 * than OSS's static `import` so older bundled Node runtimes (which gate
 * node:sqlite behind --experimental-sqlite) degrade the mirror to a
 * no-op instead of crashing every importing module on startup.
 */
...
function init(): void {
	...
	try {
		...
		// columns if a pre-refactor db still has them. The atom rows now live
...
function migrateLegacyIfNeeded(d: DatabaseSync): void {
	try {
		...
		d.exec('BEGIN');
		try {
			// All migration INSERTs filter out NULL ts_unix rows. The surface
			// tables declare ts_unix REAL NOT NULL, so a legacy row missing
			// it would trip a NOT NULL constraint mid-transaction and roll
			// the whole migration back — leaving the legacy tables in place
			// and the bot logging the same error every restart. Rows with no
			// timestamp can't be meaningfully recovered (no other column
			// carries time), so dropping them on migrate is the right call.
...
// events array as of the per-surface refactor; this filter catches
// anything missed + protects against external callers passing them in
// m.events.

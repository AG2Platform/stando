...
export function writeChatTask(taskDescription: string): string {
	...
	// /task endpoint after PR #982 / fork #32; consumers (`_isVoiceTask`,
...
export const workTool: ToolDefinition = {
	...
	async execute(args) {
		...
		// shape as agent-api.py's /task endpoint after PR #32; consumers
		...
		if (existsSync(CONTEXT_DROP_FILE)) {
			try {
				...
				if (content) {
					...
					// writers and agent-api.py's /task endpoint per PR #32.
					writeFileSync(
						join(TASK_DIR, `${taskId}.txt`),
						`id: ${taskId}\n` +

...
// Dedup window: identical task text within 2 minutes → return the existing
// taskId. Voice over-delegation pattern (Gemini calling work() 3-5 times
// with the same text in sub-second succession) used to spawn N parallel
// runs and N duplicate result DMs. Lifted from upstream PR #820 (`fix(task-
// bridge): dedup work() submissions with identical text within 2-minute
// window`); never landed in this fork. Normalization lowercases, collapses
// whitespace, and caps at 150 chars so trivial rewording (extra spaces,
// case changes, trailing punctuation past the cap) still dedupes.
...
export const workTool: ToolDefinition = {
	...
	async execute(args) {
		...
		// Catches voice over-delegation where Gemini calls work() multiple times
		// with the same text in sub-second succession.

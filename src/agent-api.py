...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # voice-originated to `_isVoiceTask` (which checks
        # `line.startswith('channel_id: local-voice')`). The misclassif
        # routes the task through the voice-only fallback path with
        # incorrect downstream behavior. Strip line terminators; cap to
        # a sane single-line length.
        from_agent = (

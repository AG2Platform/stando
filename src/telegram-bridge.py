...
def _recover_orphan_sending_files() -> int:
    ...
    But if the bridge crashes BETWEEN the rename and delivery, the
    `.sending` file sits orphaned in `results/` — no poll iteration
    ever looks at `.sending` suffixes, so the proactive is silently
    dropped. Mirrors `_recover_orphan_sending_files` in discord-bridge.py.
    """
...
def main():
    ...
    while True:
        ...
        # surprises (e.g. a Discord-context follow-up landing in
        # Telegram). See proactive_routing.py for the decision rule.

...
def _split_file_markers(text: str) -> tuple[str, list[str]]:
    ...
    Mirrors :func:`src/discord-bridge.py._split_file_markers`. ``files``
    is the list of paths extracted from ``[file:|send:|attach:]`` markers
    in textual order; ``clean_text`` is the original text with every
    marker removed and surrounding whitespace stripped.
    """

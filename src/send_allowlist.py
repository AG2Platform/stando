...
Ported from OSS sutando in Phase 5.9 of the OSS→private sync. The
policy values are byte-identical to the pre-port inline copy in
``src/discord-bridge.py`` — this extract is a pure refactor that
removes the drift hazard between the bridge and ``dm-result.py``.

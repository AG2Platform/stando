	...
	async execute(args, ctx) {
		...
		try {
			...
			if (alreadyInMeeting) {
				...
				// URL-encode the passcode — it's an arbitrary string from the
				// caller (Gemini tool arg or $ZOOM_PERSONAL_PASSCODE env). Pre-
				// fix it was spliced raw into both the URL AND a shell command,
				// so a passcode like `"; rm -rf ~/.config; #` would break out
				// of the quoted shell argument and execute. cleanId is digit-
				// only via `.replace(/\D/g, '')` above so safe; pwd needs the
				// encode pass.
				...
				// injection class entirely (no `/bin/sh -c` interpretation).

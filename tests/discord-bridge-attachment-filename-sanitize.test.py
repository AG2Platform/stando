...
e.g. `skills/phone-conversation/scripts/conversation-server.ts` line 280
(pre-fix):
...
def _load(name: str, path: Path):
    # Stub `discord` module before loading the bridge — the bridge
    # imports it at top level, but we don't need a real one for these
    # tests.
...
def test_path_traversal_strip():
    """Filename containing `..` or `/` must not let the saved path
    escape /tmp/discord-inbox/."""
...
def test_empty_filename_falls_back():
    """Pure-empty or pure-special input → fallback name (must not
    return empty, which would collapse INBOX_DIR/{ts}_ to a directory)."""
    assert sanitize("") != ""
    assert sanitize(";;;;") != ""
    assert sanitize("...") != ""
...
def test_extension_preserved_when_short():
    """Glob patterns like `/tmp/discord-inbox/*.jpg` rely on extensions
    being intact for legitimate uploads. Pin that common extensions
    survive."""
...
def test_unicode_replaced():
    """Non-ASCII chars (Unicode in filenames) get replaced with `_`.
    The bytewise position in argv is what matters; we just need a
    consistent ASCII repr."""
...
def test_overlong_filename_capped():
    """A 10KB filename uploaded as DoS would bloat the tasks-file content
    + the directory listing. Pin a length cap."""

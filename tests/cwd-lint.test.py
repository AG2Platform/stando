...
TS_ALLOWLIST: set[str] = {
    # fork: inline-tools resolves repo-relative skills/ and notes against
    # process.cwd() (voice-agent is launched from the repo). These are
    # code-adjacent lookups, NOT runtime-state paths, so resolveWorkspace()
    # would point them at the wrong tree. Pre-existing on dev-client.
    "src/inline-tools.ts",
}
